...
Then we install cuda on Linux
Ubuntu 22.04 (64-bit) support cuda 11.7+
Here we install cuda 11.7 and pytorch==1.13.1+cu117
...
1. create a new folder for the datasets in the repo  `data/`.
e.g. 
I have download the OCTID dataset from [downloaded here](BENCHMARK.md).
It was split into 3 folders: train, val and test and then
organised into 5 classes: ANormal, ARMD, CSR, Diabetic_retinopathy, Macular_Hole.

![split folder](./pic/split.png)
![label folder](./pic/label.png)
...
Run `python train_web.py` to launch the gradio app and input the paramiters.
![train](./pic/train.png)
...
You can see the training progress (Tensorboard) and log in the `task` folder.
...
![app modi](./pic/modifine.png)

This is for the task you want and choose the basic model.

![task aim](./pic/basemodel.png)

Run `python app.py` to launch the gradio app (modify the paramiters).

![appui](./pic/appui.png)

Enjoy!!!
---
**The following is the original readme file from the official repo:**
---
